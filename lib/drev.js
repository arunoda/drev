...
function Drev() {
	...
	var self = this;
	var pub, sub, commander;
	var connection = qbox.create(['pub', 'sub', 'commander']);
	...
	*/
	this.start = function(host, port, options) {
		...
		commander = redis.createClient(host, port, options);
		...
		commander.on('error', function(err) { 
